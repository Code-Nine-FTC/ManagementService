...
public class ItemLossService {
  @Autowired private ItemLossRepository itemLossRepository;
  ...
  public void createItemLoss(ItemLossRequest request, User lastUser) {
    User recordedUser =
        userRepository
            .findById(request.recordedById())
            .orElseThrow(
                () ->
                    new NullPointerException("User not found with id: " + request.recordedById()));
    Item item =
        itemRepository
            .findById(request.itemId())
            .orElseThrow(
                () -> new NullPointerException("Item not found with id: " + request.itemId()));
