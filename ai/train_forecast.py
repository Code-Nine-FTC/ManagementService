...
CREATE TABLE IF NOT EXISTS model_predictions (
    ...
    y_hat NUMERIC NOT NULL,
    model_version TEXT NOT NULL,
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
CREATE INDEX IF NOT EXISTS idx_model_predictions_ref ON model_predictions(ref_date, horizon_days);
CREATE INDEX IF NOT EXISTS idx_model_predictions_item ON model_predictions(item_id);
"""

# Tabela de métricas por previsão (por item/ref/horizon)
CREATE_METRICS_SQL = """
...
CREATE INDEX IF NOT EXISTS idx_prediction_metrics_ref ON prediction_metrics(ref_date, horizon_days);
CREATE INDEX IF NOT EXISTS idx_prediction_metrics_item ON prediction_metrics(item_id);
"""

# Tabela agregada por rodada (ref_date + horizon)
CREATE_ROUND_METRICS_SQL = """
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_model_prediction_metrics_ref_h ON model_prediction_metrics(ref_date, horizon_days);
"""

DELETE_SQL = """
DELETE FROM model_predictions WHERE ref_date = :ref_date AND horizon_days = :h;
"""

INSERT_SQL = """
INSERT INTO model_predictions (item_id, ref_date, horizon_days, y_hat, model_version, created_at)
VALUES (:item_id, :ref_date, :h, :y_hat, :model_version, NOW());
"""

SERIES_SQL = """
SELECT
  i.id AS item_id,
  DATE_TRUNC('day', COALESCE(o.withdraw_day, o.created_at))::date AS dia,
  COALESCE(SUM(oi.quantity), 0) AS quantidade
FROM items i
LEFT JOIN order_item oi ON oi.item_id = i.id
LEFT JOIN orders o ON o.id = oi.order_id
WHERE COALESCE(o.withdraw_day, o.created_at) BETWEEN :start AND :end
GROUP BY i.id, dia
ORDER BY i.id, dia;
"""
