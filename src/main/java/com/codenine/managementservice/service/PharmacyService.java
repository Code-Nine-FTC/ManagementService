...
public class PharmacyService {
    ...
    public ExpirySummary getExpirySummary(int days) {
        ...
        long expiringSoonCount = itemRepository.countExpiringSoonItemsBySection(userSectionIds, "farmácia", now, futureDate);
    ...
    public ExpiryLists getExpiryLists(int days, Pageable pageable) {
        ...
        LocalDateTime futureDate = now.plusDays(days);
        List<Long> userSectionIds = getUserSectionIds();
        List<ItemResponse> expiredItems = itemRepository.findExpiredItemsBySection(userSectionIds, "farmácia", now);
        List<ItemResponse> expiringSoonItems = itemRepository.findExpiringSoonItemsBySection(userSectionIds, "farmácia", now, futureDate);
    ...
    public ExpirySummary getAllItemsExpirySummary(int days) {
        ...
        long expiredCount = itemRepository.countAllExpiredItemsByUserSections(userSectionIds, now);
        long expiringSoonCount = itemRepository.countAllExpiringSoonItemsByUserSections(userSectionIds, now, futureDate);
    ...
    public ExpiryLists getAllItemsExpiryLists(int days, Pageable pageable) {
        ...
        LocalDateTime futureDate = now.plusDays(days).withHour(23).withMinute(59).withSecond(59);
        List<Long> userSectionIds = getUserSectionIds();
        List<ItemResponse> expiredItems = itemRepository.findAllExpiredItemsByUserSections(userSectionIds, now);
        List<ItemResponse> expiringSoonItems = itemRepository.findAllExpiringSoonItemsByUserSections(userSectionIds, now, futureDate);
    ...
    private List<Long> getUserSectionIds() {
        ...
        return user.getSections().stream()
                .map(Section::getId)
                .toList();
    }
}